...
from dmrlink import IPSC, config_reports, reportFactory, systems
...
class bridgeIPSC(IPSC):
    def __init__(self, _name, _config, _bridge_config, _logger, _report):
        ...
        self._tlvPort = (
            31003  # Port to listen on for TLV frames to transmit to all peers
        )
        self._gateway = "127.0.0.1"  # IP address of bridge app
        self._gateway_port = (
            31000  # Port bridge is listening on for TLV frames to decode
        )
    ...
    def defaultOption(self, config, sec, opt, defaultValue):
        try:
            _value = config.get(sec, opt).split(None)[
                0
            ]  # Get the value from the named section
        except ConfigParser.NoOptionError as e:
            try:
                _value = config.get("BridgeGlobal", opt).split(None)[
                    0
                ]  # Try the global BridgeGlobal section
            except ConfigParser.NoOptionError as e:
    ...
    def readConfigFile(self, configFileName, sec, networkName="BridgeGlobal"):
        ...
        try:
            ...
            self._tlvPort = int(
                self.defaultOption(config, sec, "FromGatewayPort", self._tlvPort)
            )
            self._gateway = self.defaultOption(config, sec, "Gateway", self._gateway)
            self._gateway_port = int(
                self.defaultOption(config, sec, "ToGatewayPort", self._gateway_port)
            )
    ...
    def voice_call(self, _src_id, _dst_id, _group, _ts, _end, _peerId, _rtp, _data):
        ...
        if _payload_type == BURST_DATA_TYPE["VOICE_HEADER"]:
            _stream_id = bytes_to_int(
                _data[5:6]
            )  # int8  looks like a sequence number for a packet
            if _stream_id != _tx_slot.stream_id:
                self.tlv_ipsc.begin_call(
                    _ts, _group, _src_id, _dst_id, _peerId, self.cc, _seq, _stream_id
                )
        ...
        if (_payload_type == BURST_DATA_TYPE["SLOT1_VOICE"]) or (
            ...
            self.tlv_ipsc.export_voice(
                _tx_slot,
                _seq,
                _ambe_frame1.tobytes()
                + _ambe_frame2.tobytes()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        dest="BridgeFile",
        help="/full/path/to/dmrlink_ipsc_bridge.cfg",
    )
    parser.add_argument(
        "-l",
        "--log_level",
    ...
    if not cli_args.BridgeFile:
        cli_args.BridgeFile = (
            os.path.dirname(os.path.abspath(__file__)) + "/dmrlink_ipsc_bridge.cfg"
        )
    ...
    def sig_handler(_signal, _frame):
        ...
        for system in systems:
            ...
            de_reg_req_pkt = this_ipsc.hashed_packet(
                this_ipsc._local["AuthKey"], this_ipsc.DE_REG_REQ_PKT
            )
    ...
    for system in config["Systems"]:
        if config["Systems"][system]["LOCAL"]["Enabled"]:
            systems[system] = bridgeIPSC(
                system, config, cli_args.BridgeFile, logger, report_server
            )
            reactor.listenUDP(
                config["Systems"][system]["LOCAL"]["PORT"],
                systems[system],
                interface=config["Systems"][system]["LOCAL"]["IP"],
            )
