...
from dmr_utils import ambe_utils, bptc, const, golay, lc, qr
...
class bridgeFNE(coreFNE):
    ...
    def dmrd_received(
        ...
        if (
            ...
            and (_tx_slot.type != fne_const.DT_TERMINATOR_WITH_LC)
        ):
    ...
    def get_peer_id(self, import_id):
        if (
            self._config["Mode"] == "peer"
        ):  # only peers have PeerId defined, masters do not
            ...
            for section in config.sections():
                if section == "BridgeGlobal":
                    ...
                    self._gateway = config.get(section, "Gateway").split(None)[
                        0
                    ]  # IP address of bridge app
                    self._gateway_port = int(
                        config.get(section, "ToGatewayPort").split(None)[0]
                    )  # Port bridge is listening on for AMBE frames to decode

        except ConfigParser.Error as err:
            traceback.print_exc()
            sys.exit(
                "Could not parse configuration file, " + _file_name + ", exiting..."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        dest="BridgeFile",
        help="/full/path/to/fne_bridge.cfg",
    )
    parser.add_argument(
        "-l",
        "--logging",
    ...
    if not cli_args.BridgeFile:
        cli_args.BridgeFile = (
            os.path.dirname(os.path.abspath(__file__)) + "/fne_bridge.cfg"
        )
    ...
    for system in config["Systems"]:
        if config["Systems"][system]["Enabled"]:
            systems[system] = bridgeFNE(
                system, config, cli_args.BridgeFile, logger, act_log_file, report_server
            )
            reactor.listenUDP(
