...
def make_rules(_fne_routing_rules):
    ...
    for _system in rule_file.RULES:
        if _system not in config["Systems"]:
            logger.error(
                "Routing rules found for system %s, not configured main configuration",
                _system,
            )
            continue
        ...
        for _rule in rule_file.RULES[_system]["GROUP_VOICE"]:
            ...
            logger.info(
                "Rule (%s) NAME: %s SRC_TGID: %s DST_TGID: %s SRC_TS: %s DST_TS: %s ACTIVE: %s ROUTABLE: %s TO_TYPE: %s AFFILIATED: %s IGNORED: %s",
                _system,
                _rule["NAME"],
                _rule["SRC_GROUP"],
                _rule["DST_GROUP"],
            ...
            if _rule["TO_TYPE"] == "ON":
                if _rule["ROUTABLE"] == True:
                    if _rule["TIMER"] < _now:
                        ...
                        logger.info(
                            "(%s) TG Routing timeout DEACTIVATE routing name %s, Target %s, TS %s, TGID %s",
                            _system,
                            _rule["NAME"],
                            _rule["DST_NET"],
                            _rule["DST_TS"],
                        ...
                        logger.info(
                            "(%s) TG Routing ACTIVE with ON timer running Timeout eligible in %ds, Rule name %s, Target %s, TS %s, TGID %s",
                            _system,
                            timeout_in,
                            _rule["NAME"],
                            _rule["DST_NET"],
                ...
                if _rule["ROUTABLE"] == False:
                    if _rule["TIMER"] < _now:
                        _rule["ROUTABLE"] = True
                        logger.info(
                            "(%s) TG Routing timeout ACTIVATE Rule name %s, Target %s, TS %s, TGID %s",
                            _system,
                            _rule["NAME"],
                            _rule["DST_NET"],
                            _rule["DST_TS"],
                        ...
                        timeout_in = _rule["TIMER"] - _now
                        logger.info(
                            "(%s) TG Routing DEACTIVATE with OFF timer running Timeout eligible in %ds, Rule name %s, Target %s, TS %s, TGID %s",
                            _system,
                            timeout_in,
                            _rule["NAME"],
                            _rule["DST_NET"],
...
class routerFNE(coreFNE):
    ...
    def dmrd_validate(
        ...
        if get_valid(_rf_src, black_rids) == True:
            if _stream_id != self.STATUS[_slot]["RX_STREAM_ID"]:
                ...
                self._logger.warning(
                    "(%s) DMRD: Traffic *REJECT ACL      * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s] (Blacklisted RID)",
                    self._system,
                    _peer_id,
                    _rf_src,
                    _dst_id,
                ...
                if config["Reports"]["Report"]:
                    self._report.send_routeEvent(
                        "REJECT ACL,BLACKLISTED RID,DMR,{},{},{},{},{},{}".format(
                            self._system, _stream_id, _peer_id, _rf_src, _slot, _dst_id
                        )
                    )
            return False
        ...
        if (_frame_type == fne_const.FT_DATA_SYNC) and (
            _dtype_vseq == fne_const.DT_TERMINATOR_WITH_LC
        ):
            return True
        ...
        if _call_type == "group":
            if (RULES[self._system]["SEND_TGID"] == True) and (
                ...
                if _stream_id != self.STATUS[_slot]["RX_STREAM_ID"]:
                    ...
                    self.STATUS[_slot]["RX_STREAM_ID"] = _stream_id
                    self._logger.warning(
                        "(%s) DMRD: Traffic *REJECT ACL      * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s] (Illegal TGID)",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "REJECT ACL,ILLEGAL TGID,DMR,{},{},{},{},{},{}".format(
                                self._system,
                                _stream_id,
                                _peer_id,
                                _rf_src,
    ...
    def dmrd_received(
        ...
        if (_frame_type == fne_const.FT_DATA_SYNC) and (
            (_dtype_vseq == fne_const.DT_DATA_HEADER)
            or (_dtype_vseq == fne_const.DT_RATE_12_DATA)
            or (_dtype_vseq == fne_const.DT_RATE_34_DATA)
            or (_dtype_vseq == fne_const.DT_RATE_1_DATA)
        ):
            self._logger.info(
                "(%s) DMRD: Traffic *DATA            * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s]",
                self._system,
                _peer_id,
                _rf_src,
                _dst_id,
        ...
        if _call_type == "group":
            ...
            if _stream_id != self.STATUS[_slot]["RX_STREAM_ID"]:
                if (
                    (self.STATUS[_slot]["RX_TYPE"] != fne_const.DT_TERMINATOR_WITH_LC)
                    and (
                        pkt_time < (self.STATUS[_slot]["RX_TIME"] + fne_const.STREAM_TO)
                    )
                    and (_rf_src != self.STATUS[_slot]["RX_RFS"])
                ):
                    self._logger.warning(
                        "(%s) DMRD: Traffic *CALL COLLISION  * PEER %s SRC_ID %s TGID %s TS %s [STREAM ID %s] (Collided with existing call)",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "GROUP VOICE,CALL COLLISION,DMR,{},{},{},{},{},{}".format(
                                self._system,
                                _stream_id,
                                _peer_id,
                                _rf_src,
                ...
                # options intact
                if (
                    _frame_type == fne_const.FT_DATA_SYNC
                    and _dtype_vseq == fne_const.DT_VOICE_LC_HEADER
                ):
                ...
                # enough, and it saves lots of time
                    ...
                    self.STATUS[_slot]["RX_LC"] = (
                        const.LC_OPT + short_to_bytes(_dst_id) + short_to_bytes(_rf_src)
                    )

                self.STATUS[_slot]["RX_PI_LC"] = (
                    const.LC_PI_OPT + b"\x00\x00\x00" + b"\x00\x00"
                )
                self._logger.debug(
                    "(%s) TS %s [STREAM ID %s] RX_LC %s",
                    self._system,
                    _slot,
                    _stream_id,
                    ahex(self.STATUS[_slot]["RX_LC"]),
                )
            ...
            if (
                _frame_type == fne_const.FT_DATA_SYNC
                and _dtype_vseq == fne_const.DT_VOICE_PI_HEADER
            ):
                lcHeader = lc.decode_lc_header(dmrpkt)
                _alg_id = lcHeader["LC"][0] & 0x7
                _key_id = lcHeader["LC"][2]
                self._logger.info(
                    "(%s) DMRD: Traffic *CALL PI PARAMS  * PEER %s DST_ID %s TS %s ALGID %s KID %s [STREAM ID %s]",
                    self._system,
                    _peer_id,
                    _dst_id,
                    _slot,
                ...
                self._logger.debug(
                    "(%s) TS %s [STREAM ID %s] RX_PI_LC %s",
                    self._system,
                    _slot,
                    _stream_id,
                    ahex(self.STATUS[_slot]["RX_PI_LC"]),
                )
            ...
            for rule in RULES[self._system]["GROUP_VOICE"]:
                ...
                if (
                    ...
                    #
                    if (
                        rule["DST_GROUP"] != _target_status[rule["DST_TS"]]["RX_TGID"]
                    ) and (
                        ...
                        if (
                            ...
                            self._logger.info(
                                "(%s) DMRD: Call not routed to TGID %s, target active or in group hangtime: PRID %s TS %s TGID %s",
                                self._system,
                                rule["DST_GROUP"],
                                _target,
                                rule["DST_TS"],
                        ...
                        continue
                    if (
                        rule["DST_GROUP"] != _target_status[rule["DST_TS"]]["TX_TGID"]
                    ) and (
                        ...
                        if (
                            ...
                            self._logger.info(
                                "(%s) DMRD: Call not routed to TGID %s, target in group hangtime: PRID %s TS %s TGID %s",
                                self._system,
                                rule["DST_GROUP"],
                                _target,
                                rule["DST_TS"],
                    ...
                    if (
                        ...
                        if (
                            ...
                            self._logger.info(
                                "(%s) DMRD: Call not routed to TGID %s, matching call already active on target: PRID %s TS %s TGID %s",
                                self._system,
                                rule["DST_GROUP"],
                                _target,
                                rule["DST_TS"],
                    ...
                    if (
                        (rule["DST_GROUP"] == _target_status[rule["DST_TS"]]["TX_TGID"])
                        and (_rf_src != _target_status[rule["DST_TS"]]["TX_RFS"])
                        and (
                            (pkt_time - _target_status[rule["DST_TS"]]["TX_TIME"])
                            < fne_const.STREAM_TO
                        )
                        ...
                        if (
                            ...
                            self._logger.info(
                                "(%s) DMRD: Call not routed for SUB %s, call route in progress on target: PRID %s TS %s TGID %s SUB %s",
                                self._system,
                                _rf_src,
                                _target,
                                rule["DST_TS"],
                    ...
                    if (
                        ...
                        or (_target_status[rule["DST_TS"]]["TX_RFS"] != _rf_src)
                        or (
                            _target_status[rule["DST_TS"]]["TX_TGID"]
                            != rule["DST_GROUP"]
                        )
                        ...
                        dst_lc = (
                            self.STATUS[_slot]["RX_LC"][0:3]
                            + short_to_bytes(rule["DST_GROUP"])
                            + short_to_bytes(_rf_src)
                        )
                        ...
                        _target_status[rule["DST_TS"]]["TX_EMB_LC"] = bptc.encode_emblc(
                            dst_lc
                        )

                        dst_pi_lc = (
                            self.STATUS[_slot]["RX_PI_LC"][0:7]
                            + short_to_bytes(rule["DST_GROUP"])
                            + b"\x00\x00"
                        )
                        _target_status[rule["DST_TS"]][
                            "TX_P_LC"
                        ] = bptc.encode_header_pi(dst_pi_lc)

                        self._logger.debug(
                            "(%s) TS %s [STREAM ID %s] TX_H_LC %s",
                            self._system,
                            _slot,
                            _stream_id,
                            ahex(dst_lc),
                        ...
                        self._logger.debug(
                            "(%s) DMR Packet DST TGID %s does not match SRC TGID %s - Generating FULL and EMB LCs",
                            self._system,
                            rule["DST_GROUP"],
                            _dst_id,
                        )
                        self._logger.info(
                            "(%s) DMRD: Call routed to SYSTEM %s TS %s TGID %s",
                            self._system,
                            _target,
                            rule["DST_TS"],
                            rule["DST_GROUP"],
                    ...
                    if (_pi_dst_id != 0) and (
                        _target_status[rule["DST_TS"]]["TX_PI_TGID"]
                        != rule["DST_GROUP"]
                    ):
                        # Record the DST TGID and Stream ID
                        ...
                        self._logger.info(
                            "(%s) DMRD: Call PI parameters routed to SYSTEM %s TS %s TGID %s",
                            self._system,
                            _target,
                            rule["DST_TS"],
                            rule["DST_GROUP"],
                    ...
                    if _tgt_peer_id == 0:
                        self._logger.warning(
                            "(%s) DMRD: Some how we got a 0 peer ID? This shouldn't happen, spoofing to %s",
                            self._system,
                            _peer_id,
                        )
                    ...
                    _tmp_data = (
                        _data[:8]
                        + short_to_bytes(rule["DST_GROUP"])
                        + int_to_bytes(_tgt_peer_id)
                        + _tmp_bits.to_bytes(1, "big")
                        + _data[16:20]
                    )
                    ...
                    if (
                        ...
                        dmrbits = (
                            _target_status[rule["DST_TS"]]["TX_H_LC"][0:98]
                            + dmrbits[98:166]
                            + _target_status[rule["DST_TS"]]["TX_H_LC"][98:197]
                        )
                    # Create a voice PI header packet (FULL LC)
                        ...
                        dmrbits = (
                            _target_status[rule["DST_TS"]]["TX_P_LC"][0:98]
                            + dmrbits[98:166]
                            + _target_status[rule["DST_TS"]]["TX_P_LC"][98:197]
                        )
                    # Create a voice terminator packet (FULL LC)
                    elif (
                        _frame_type == fne_const.FT_DATA_SYNC
                        and _dtype_vseq == fne_const.DT_TERMINATOR_WITH_LC
                    ):
                        dmrbits = (
                            _target_status[rule["DST_TS"]]["TX_T_LC"][0:98]
                            + dmrbits[98:166]
                            + _target_status[rule["DST_TS"]]["TX_T_LC"][98:197]
                        )
                    # Create a Burst B-E packet (Embedded LC)
                    elif _dtype_vseq in [1, 2, 3, 4]:
                        dmrbits = (
                            dmrbits[0:116]
                            + _target_status[rule["DST_TS"]]["TX_EMB_LC"][_dtype_vseq]
                            + dmrbits[148:264]
                        )
                    ...
                    self._logger.debug(
                        "(%s) DMR Packet routed by rule %s to %s SYSTEM %s",
                        self._system,
                        rule["NAME"],
                        self._CONFIG["Systems"][_target]["Mode"],
                        _target,
                    )
            ...
            if (
                (_frame_type == fne_const.FT_DATA_SYNC)
                and (_dtype_vseq == fne_const.DT_TERMINATOR_WITH_LC)
                and (self.STATUS[_slot]["RX_TYPE"] != fne_const.DT_TERMINATOR_WITH_LC)
            ):
                ...
                for rule in RULES[self._system]["GROUP_VOICE"]:
                    ...
                    if (
                        ...
                        self._logger.info(
                            "(%s) DMRD: Source group transmission match for rule %s. Reset timeout to %s",
                            self._system,
                            rule["NAME"],
                            rule["TIMER"],
                        )
                        ...
                        for target_rule in RULES[_target]["GROUP_VOICE"]:
                            if target_rule["NAME"] == rule["NAME"]:
                                ...
                                self._logger.info(
                                    "(%s) DMRD: Reciprocal group transmission match for rule %s on IPSC %s. Reset timeout to %s",
                                    self._system,
                                    target_rule["NAME"],
                                    _target,
                                    rule["TIMER"],
                    ...
                    if _dst_id in rule["ON"]:
                        ...
                        rule["TIMER"] = pkt_time + rule["TIMEOUT"]
                        self._logger.info(
                            "(%s) DMRD: Primary routing Rule %s changed to state: %s",
                            self._system,
                            rule["NAME"],
                            rule["ROUTABLE"],
                        )
                        ...
                        for target_rule in RULES[_target]["GROUP_VOICE"]:
                            if target_rule["NAME"] == rule["NAME"]:
                                ...
                                target_rule["TIMER"] = pkt_time + target_rule["TIMEOUT"]
                                self._logger.info(
                                    "(%s) DMRD: Reciprocal routing Rule %s in IPSC %s changed to state: %s",
                                    self._system,
                                    target_rule["NAME"],
                                    _target,
                                    rule["ROUTABLE"],
                    ...
                    if _dst_id in rule["OFF"]:
                        ...
                        rule["ROUTABLE"] = False
                        self._logger.info(
                            "(%s) DMRD: Routing Rule %s changed to state: %s",
                            self._system,
                            rule["NAME"],
                            rule["ROUTABLE"],
                        )
                        ...
                        for target_rule in RULES[_target]["GROUP_VOICE"]:
                            if target_rule["NAME"] == rule["NAME"]:
                                ...
                                self._logger.info(
                                    "(%s) DMRD: DMR Reciprocal routing Rule %s in IPSC %s changed to state: %s",
                                    self._system,
                                    target_rule["NAME"],
                                    _target,
                                    rule["ROUTABLE"],
            ...
            if _stream_id != self.STATUS[_slot]["RX_STREAM_ID"]:
                if (
                    ...
                    self._logger.warning(
                        "(%s) DMRD: Traffic *CALL COLLISION  * PEER %s SRC_ID %s DST_ID %s TS %s [STREAM ID %s] (Collided with existing call)",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "PRV VOICE,CALL COLLISION,DMR,{},{},{},{},{},{}".format(
                                self._system,
                                _stream_id,
                                _peer_id,
                                _rf_src,
                ...
                self.STATUS[_slot]["RX_START"] = pkt_time
                self._logger.info(
                    "(%s) DMRD: Traffic *PRV CALL START  * PEER %s SRC_ID %s DST_ID %s TS %s [STREAM ID %s]",
                    self._system,
                    _peer_id,
                    _rf_src,
                    _dst_id,
            ...
            if (
                ...
                call_duration = pkt_time - self.STATUS[_slot]["RX_START"]
                self._logger.info(
                    "(%s) DMRD: Traffic *PRV CALL END    * PEER %s SRC_ID %s DST_ID %s TS %s DUR %s [STREAM ID: %s]",
                    self._system,
                    _peer_id,
                    _rf_src,
                    _dst_id,
        ...
        if (_duid == fne_const.P25_DUID_TSDU) or (_duid == fne_const.P25_DUID_PDU):
            if _duid == fne_const.P25_DUID_TSDU:
                if _lcf == fne_const.P25_TSBK_IOSP_GRP_AFF:
                    self._logger.info(
                        "(%s) P25D: Traffic *TSBK GRP AFF    * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s]",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "TSBK,GRP AFF,P25,{},{},{},{},{},{}".format(
                                self._system, _stream_id, _peer_id, _rf_src, 1, _dst_id
                            )
                        )
                elif _lcf == fne_const.P25_TSBK_OSP_U_DEREG_ACK:
                    self._logger.info(
                        "(%s) P25D: Traffic *TSBK U DEREG ACK* PEER %s SRC_ID %s [STREAM ID %s]",
                        self._system,
                        _peer_id,
                        _dst_id,
                        _stream_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "TSBK,U DEREG ACK,P25,{},{},{},{},{},{}".format(
                                self._system, _stream_id, _peer_id, _rf_src, 1, _dst_id
                            )
                        )
                elif _lcf == fne_const.P25_TSBK_OSP_ADJ_STS_BCAST:
                    self._logger.info(
                        "(%s) P25D: Traffic *TSBK ADJ STS BCS* PEER %s [STREAM ID %s]",
                        self._system,
                        _peer_id,
                        _stream_id,
                    )
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "TSBK,ADJ STS BCS,P25,{},{},{},{},{},{}".format(
                                self._system, _stream_id, _peer_id, _rf_src, 1, _dst_id
                            )
                        )
                elif _lcf == fne_const.P25_LCF_TSBK_CALL_ALERT:
                    self._logger.info(
                        "(%s) P25D: Traffic *TSBK CALL ALERT * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s]",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "TSBK,CALL ALERT,P25,{},{},{},{},{},{}".format(
                                self._system, _stream_id, _peer_id, _rf_src, 1, _dst_id
                            )
                        )
                elif _lcf == fne_const.P25_LCF_TSBK_ACK_RSP_FNE:
                    self._logger.info(
                        "(%s) P25D: Traffic *TSBK ACK RSP    * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s]",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "TSBK,ACK RSP,P25,{},{},{},{},{},{}".format(
                                self._system, _stream_id, _peer_id, _rf_src, 1, _dst_id
                            )
                        )
            elif _duid == fne_const.P25_DUID_PDU:
                self._logger.info(
                    "(%s) P25D: Traffic *DATA            * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s]",
                    self._system,
                    _peer_id,
                    _rf_src,
                    _dst_id,
    ...
    def p25d_validate(
        self, _peer_id, _rf_src, _dst_id, _call_type, _duid, _dtype_vseq, _stream_id
    ):
        pkt_time = time()
        _slot = 1
        ...
        if get_valid(_rf_src, black_rids) == True:
            if _stream_id != self.STATUS[_slot]["RX_STREAM_ID"]:
                ...
                self.STATUS[_slot]["RX_STREAM_ID"] = _stream_id
                self._logger.warning(
                    "(%s) P25D: Traffic *REJECT ACL      * PEER %s SRC_ID %s DST_ID %s DUID %s [STREAM ID %s] (Blacklisted RID)",
                    self._system,
                    _peer_id,
                    _rf_src,
                    _dst_id,
                ...
                if config["Reports"]["Report"]:
                    self._report.send_routeEvent(
                        "REJECT ACL,BLACKLISTED RID,P25,{},{},{},{},{},{}".format(
                            self._system, _stream_id, _peer_id, _rf_src, 1, _dst_id
                        )
                    )
            return False
        ...
        if _call_type == "group":
            if (RULES[self._system]["SEND_TGID"] == True) and (
                ...
                if _stream_id != self.STATUS[_slot]["RX_STREAM_ID"]:
                    ...
                    self._logger.warning(
                        "(%s) P25D: Traffic *REJECT ACL      * PEER %s SRC_ID %s DST_ID %s DUID %s [STREAM ID %s] (Illegal TGID)",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "REJECT ACL,ILLEGAL TGID,P25,{},{},{},{},{},{}".format(
                                self._system, _stream_id, _peer_id, _rf_src, 1, _dst_id
                            )
                        )
                return False
            ...
            if (
                ...
                if _stream_id != self.STATUS[_slot]["RX_STREAM_ID"]:
                    ...
                    self._logger.warning(
                        "(%s) P25D: Traffic *REJECT ACL      * PEER %s SRC_ID %s DST_ID %s DUID %s [STREAM ID %s] (Illegal RID)",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "REJECT ACL,ILLEGAL RID,P25,{},{},{},{},{},{}".format(
                                self._system, _stream_id, _peer_id, _rf_src, 1, _dst_id
                            )
                        )
                return False
    ...
    def p25d_received(
        ...
        if (
            (_duid == fne_const.P25_DUID_TDU) or (_duid == fne_const.P25_DUID_TDULC)
        ) and (self.STATUS[_slot]["RX_TYPE"] != fne_const.DT_TERMINATOR_WITH_LC):
        ...
        if _call_type == "group":
            ...
            if (_stream_id != self.STATUS[_slot]["RX_STREAM_ID"]) and (
                (_duid != fne_const.P25_DUID_TDU)
                and (_duid != fne_const.P25_DUID_TDULC)
            ):
                if (
                    (self.STATUS[_slot]["RX_TYPE"] != fne_const.DT_TERMINATOR_WITH_LC)
                    and (
                    ...
                    self._logger.warning(
                        "(%s) P25D: Traffic *CALL COLLISION  * PEER %s SRC_ID %s TGID %s [STREAM ID %s] (Collided with existing call)",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "GROUP VOICE,CALL COLLISION,P25,{},{},{},{},{},{}".format(
                                self._system,
                                _stream_id,
                                _peer_id,
                                _rf_src,
            ...
            for rule in RULES[self._system]["GROUP_VOICE"]:
                ...
                if (
                    ...
                    if (
                        ...
                        self._logger.info(
                            "(%s) P25D: Call not routed to TGID %s, target active or in group hangtime: PRID %s TGID %s",
                            self._system,
                            rule["DST_GROUP"],
                            _target,
                            _target_status[rule["DST_TS"]]["RX_TGID"],
                        ...
                        if config["Reports"]["Report"]:
                            self._report.send_routeEvent(
                                "CALL ROUTE,FAILED,P25,{},{},{},{}".format(
                                    self._system, _target, 1, rule["DST_GROUP"]
                                )
                            )
                        continue
                    if (
                        ...
                        self._logger.info(
                            "(%s) P25D: Call not routed to TGID %s, target in group hangtime: PRID %s TGID %s",
                            self._system,
                            rule["DST_GROUP"],
                            _target,
                            _target_status[rule["DST_TS"]]["TX_TGID"],
                    ...
                    if (
                        ...
                        self._logger.info(
                            "(%s) P25D: Call not routed for SRC_ID %s, call route in progress on target: PRID %s TGID %s SRC_ID %s",
                            self._system,
                            _rf_src,
                            _target,
                            _target_status[rule["DST_TS"]]["TX_TGID"],
                    ...
                    if (
                        ...
                        _target_status[rule["DST_TS"]]["TX_RFS"] = _rf_src
                        self._logger.info(
                            "(%s) P25D: Call routed to SYSTEM %s TGID %s",
                            self._system,
                            _target,
                            rule["DST_GROUP"],
                        )
                    ...
                    if _tgt_peer_id == 0:
                        self._logger.warning(
                            "(%s) P25D: Some how we got a 0 peer ID? This shouldn't happen, spoofing to %s",
                            self._system,
                            _peer_id,
                        )
                        _tgt_peer_id = _peer_id
                    ...
                    systems[_target].send_system(_tmp_data)
                    self._logger.debug(
                        "(%s) P25 Packet routed by rule %s to %s SYSTEM %s",
                        self._system,
                        rule["NAME"],
                        self._CONFIG["Systems"][_target]["Mode"],
                        _target,
            ...
            if (
                ...
                for rule in RULES[self._system]["GROUP_VOICE"]:
                    ...
                    if _dst_id == rule["SRC_GROUP"] and (
                        ...
                        rule["TIMER"] = pkt_time + rule["TIMEOUT"]
                        self._logger.info(
                            "(%s) P25D: Source group transmission match for rule %s. Reset timeout to %s",
                            self._system,
                            rule["NAME"],
                            rule["TIMER"],
                        )
                        ...
                        for target_rule in RULES[_target]["GROUP_VOICE"]:
                            if target_rule["NAME"] == rule["NAME"]:
                                target_rule["TIMER"] = pkt_time + target_rule["TIMEOUT"]
                                self._logger.info(
                                    "(%s) P25D: Reciprocal group transmission match for rule %s on IPSC %s. Reset timeout to %s",
                                    self._system,
                                    target_rule["NAME"],
                                    _target,
                                    rule["TIMER"],
                    ...
                    if _dst_id in rule["ON"]:
                        ...
                        rule["TIMER"] = pkt_time + rule["TIMEOUT"]
                        self._logger.info(
                            "(%s) P25D: Primary routing Rule %s changed to state: %s",
                            self._system,
                            rule["NAME"],
                            rule["ROUTABLE"],
                        )
                        ...
                        for target_rule in RULES[_target]["GROUP_VOICE"]:
                            if target_rule["NAME"] == rule["NAME"]:
                                ...
                                target_rule["TIMER"] = pkt_time + target_rule["TIMEOUT"]
                                self._logger.info(
                                    "(%s) P25D: Reciprocal routing Rule %s in IPSC %s changed to state: %s",
                                    self._system,
                                    target_rule["NAME"],
                                    _target,
                                    rule["ROUTABLE"],
                    ...
                    if _dst_id in rule["OFF"]:
                        ...
                        rule["ROUTABLE"] = False
                        self._logger.info(
                            "(%s) P25D: Routing Rule %s changed to state: %s",
                            self._system,
                            rule["NAME"],
                            rule["ROUTABLE"],
                        )
            ...
            if (_stream_id != self.STATUS[_slot]["RX_STREAM_ID"]) and (
                ...
                if (
                    ...
                    self._logger.warning(
                        "(%s) P25D: Traffic *CALL COLLISION  * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s] (Collided with existing call)",
                        self._system,
                        _peer_id,
                        _rf_src,
                        _dst_id,
                    ...
                    if config["Reports"]["Report"]:
                        self._report.send_routeEvent(
                            "PRV VOICE,CALL COLLISION,P25,{},{},{},{},{},{}".format(
                                self._system,
                                _stream_id,
                                _peer_id,
                                _rf_src,
                ...
                self.STATUS[_slot]["RX_START"] = pkt_time
                self._logger.info(
                    "(%s) P25D: Traffic *PRV CALL START  * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s]",
                    self._system,
                    _peer_id,
                    _rf_src,
                    _dst_id,
            ...
            if (
                ...
                _rf_src = self.STATUS[_slot]["RX_RFS"]
                self._logger.info(
                    "(%s) P25D: Traffic *PRV CALL END    * PEER %s SRC_ID %s DST_ID %s DUR %s [STREAM ID %s]",
                    self._system,
                    _peer_id,
                    _rf_src,
                    _dst_id,
    ...
    def peer_ignored(
        ...
        if (
            ...
            if _stream_id != self.STATUS[_slot]["RX_STREAM_ID"]:
                ...
                self._logger.warning(
                    "(%s) Traffic *REJECT ACL      * PEER %s SRC_ID %s DST_ID %s [STREAM ID %s] (Ignored Peer)",
                    self._system,
                    _peer_id,
                    _rf_src,
                    _dst_id,
                ...
                if config["Reports"]["Report"]:
                    self._report.send_routeEvent(
                        "REJECT ACL,IGNORED PEER,ACL,{},{},{},{},{},{}".format(
                            self._system, _stream_id, _peer_id, _rf_src, _slot, _dst_id
                        )
                    )
    ...
    def update_grp_aff(self, _peer_id, _rf_src, _dst_id, _stream_id):
        ...
        if not _dst_id in GRP_AFF[_peer_id].keys():
            ...
            self._logger.info(
                "(%s) P25D: PEER %s Added TGID %s to affiliations table [STREAM ID %s]",
                self._system,
                _peer_id,
                _dst_id,
                _stream_id,
        ...
        self._logger.info(
            "(%s) P25D: PEER %s Added SRC_ID %s affiliation to TGID %s [STREAM ID %s]",
            self._system,
            _peer_id,
            _rf_src,
            _dst_id,
    ...
    def remove_grp_aff(self, _peer_id, _rf_src, _stream_id):
        ...
        for tgid in GRP_AFF[_peer_id]:
            if _rf_src in GRP_AFF[_peer_id][tgid]:
                try:
                    ...
                    self._logger.info(
                        "(%s) P25D: PEER %s Removed SRC_ID %s affiliation from TGID %s [STREAM ID %s]",
                        self._system,
                        _peer_id,
                        _rf_src,
                        tgid,
        ...
        if _dst_id != 0:
            ...
            self._logger.info(
                "(%s) P25D: PEER %s Removed TGID %s from affiliations table [STREAM ID %s]",
                self._system,
                _peer_id,
                _dst_id,
                _stream_id,
    ...
    def rid_tid_update_loop(self):
        ...
        try:
            ...
            if RULES[self._system]["SEND_TGID"] == True:
                ...
                if _tg_ids:
                    self._logger.debug(
                        "ID MAPPER: tg_ids dictionary is available, and being sent to peers"
                    )
                ...
                if _deactive_tg_ids:
                    self._logger.debug(
                        "ID MAPPER: deactive_tg_ids dictionary is available, and being sent to peers"
                    )
        ...
        white_rids = mk_id_dict(
            self._CONFIG["Aliases"]["Path"],
            self._CONFIG["Aliases"]["WhitelistRIDsFile"],
        )
        if white_rids:
            self._logger.debug(
                "ID MAPPER: white_rids dictionary is available, and being sent to peers"
            )
        ...
        black_rids = mk_id_dict(
            self._CONFIG["Aliases"]["Path"],
            self._CONFIG["Aliases"]["BlacklistRIDsFile"],
        )
        if black_rids:
            self._logger.debug(
                "ID MAPPER: black_rids dictionary is available, and being sent to peers"
            )
...
if __name__ == "__main__":
    from fne.fne_core import mk_id_dict, setup_fne

    # perform basic FNE setup
    config, logger, act_log_file = setup_fne()
    logger.info("Digital Voice Modem FNE Router Service R02.50")
    ...
    for system in config["Systems"]:
        if config["Systems"][system]["Enabled"]:
            systems[system] = routerFNE(
                system, config, logger, act_log_file, report_server
            )
            reactor.listenUDP(
                config["Systems"][system]["Port"],
                systems[system],
                interface=config["Systems"][system]["Address"],
            )
            logger.debug(
                "%s instance created: %s, %s",
                config["Systems"][system]["Mode"],
                system,
                systems[system],
            )
