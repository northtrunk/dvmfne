...
from time import clock, localtime, sleep, strftime, time

import ambe_utils
from bitarray import bitarray
from bitstring import BitArray, BitString
from dmr_utils import bptc, const, golay, lc, qr, rs129
...
"""
lc_header_flag = lambda _slot: (0xA0 if (_slot == 2) else 0x20) | ord(
    const.DT_VOICE_LC_HEADER
)
pi_header_flag = lambda _slot: (0xA0 if (_slot == 2) else 0x20) | ord(
    const.DT_VOICE_PI_HEADER
)
terminator_flag = lambda _slot: (0xA0 if (_slot == 2) else 0x20) | ord(
    const.DT_TERMINATOR_WITH_LC
)
voice_flag = (
    lambda _slot, _vf: (0x80 if (_slot == 2) else 0) | (0x10 if (_vf == 0) else 0) | _vf
)
...
class SLOT:
    def __init__(self, _slot, _src_id, _dst_id, _peer_id, _cc):
        self.src_id = _src_id  # Source ID
        self.dst_id = _dst_id  # Destination ID (TG)
        self.peer_id = _peer_id  # Peer ID
        self.slot = _slot  # Slot to use
        self.cc = _cc  # Color code to use
        self.type = (
            0  # 1=voice header, 2=voice terminator; voice, 0=burst A ... 5=burst F
        )
        self.stream_id = 0  # Stream id is same across a single session
...
class tlvBase:
    def __init__(self, _parent, _name, _config, _logger, _port):
        ...
        self._tlvPort = (
            _port  # Port to listen on for TLV frames to transmit to all peers
        )
        ...
        self.udp_port = reactor.listenUDP(
            self._tlvPort, UDP_IMPORT(self.import_datagramReceived)
        )
        pass
        ...
        if t:
            ...
            if l:
                ...
                if v:
                    ...
                    if t == TAG_BEGIN_TX:
                        if ord(l) > 1:
                            _slot = bytes_to_int(v[10:11])
                            _rx_slot = self.rx[_slot]
                            _rx_slot.slot = _slot
                            _rx_slot.src_id = bytes_to_int(v[0:3])
                            _rx_slot.peer_id = self._parent.get_peer_id(
                                bytes_to_int(v[3:7])
                            )
                            _rx_slot.dst_id = bytes_to_int(v[7:10])
                            _rx_slot.cc = bytes_to_int(v[11:12])
                        ...
                        _rx_slot.stream_id = randint(
                            0, 0xFFFFFFFF
                        )  # Every stream has a unique ID
                        self._logger.info(
                            "(%s) TLV BEGIN_TX, STREAM ID %s SRC_ID %s PEER %s GROUP %s TGID %s TS %s",
                            self._system,
                            _rx_slot.stream_id,
                            _rx_slot.src_id,
                            _rx_slot.peer_id,
                        ...
                        if ord(l) > 1:
                            _slot = bytes_to_int(v[9:10])
                            _rx_slot = self.rx[_slot]
                            _rx_slot.secure = True
                            _rx_slot.dst_id = bytes_to_int(v[0:3])
                            _rx_slot.alg_id = bytes_to_int(v[3:4]) & 0x7
                            _rx_slot.key_id = bytes_to_int(v[4:5])
                            _rx_slot.mi = v[5:9]
                        self._logger.info(
                            "(%s) TLV PI_INFO, STREAM ID %s SRC_ID %s PEER %s TS %s ALG %s KID %s",
                            self._system,
                            _rx_slot.stream_id,
                            _rx_slot.src_id,
                            _rx_slot.peer_id,
                        ...
                        self._logger.info(
                            "(%s) TLV END_TX, STREAM ID %d FRAMES %d",
                            self._system,
                            _rx_slot.stream_id,
                            _rx_slot.frame_count,
                        )
                        ...
                        _rx_slot.dst_id = int(v.split("=")[1])
                        self._logger.info(
                            "(%s) TLV DMR_TEST, TGID %d TS %d",
                            self._system,
                            _rx_slot.dst_id,
                            _rx_slot.slot,
                        )
                        ...
                        thread.start_new_thread(
                            self.sendBlankAmbe,
                            (_rx_slot, randint(0, 0xFFFFFFFF), 5 * 60 * 500),
                        )

                    else:
                        self._logger.info(
                            "(%s) TLV unknown, T %d L %d, V %s",
                            self._system,
                            t,
                            ord(l),
                            ahex(v),
    ...
    def sendBlankAmbe(self, _rx_slot, _stream_id, _frames=1):
        ...
        self.send_voice_header(_rx_slot)
        sleep(0.06)
        silence = b"\xAC\AA\x40\x20\x00\x44\x40\x80\x80"
    ...
    def begin_call(
        self, _slot, _group_call, _src_id, _dst_id, _peer_id, _cc, _seq, _stream_id
    ):
        group = b"\x01"
        ...
        self._logger.info(
            "Voice Transmission Start; slot = {}, dstId = {}, srcId = {}".format(
                _slot, _dst_id, _src_id
            )
        )
    ...
    def pi_params(self, _slot, _dst_id, _alg_id, _key_id, _mi):
        metadata = (
            _dst_id[0:3]
            + int_to_bytes(_alg_id)
            + int_to_bytes(_key_id)
            + _mi[0:4]
            + struct.pack("B", _slot)
        )
        ...
        self._logger.info(
            "PI parameters; slot = {}, dstId = {}, algId = {}, kId = {}".format(
                _slot, _dst_id, _alg_id, _key_id
            )
        )
    ...
    def end_call(self, _tx_slot):
        ...
        _lost_percentage = (
            ((_tx_slot.lostFrame / float(_tx_slot.frame_count)) * 100.0)
            if _tx_slot.frame_count > 0
            else 0.0
        )

        self._logger.info(
            "Voice Transmission End; {:.2f} seconds loss rate: {:.2f}% ({}/{})".format(
                call_duration,
                _lost_percentage,
                _tx_slot.frame_count - _tx_slot.lostFrame,
                _tx_slot.frame_count,
            )
        )
...
class tlvFNE(tlvBase):
    ...
    def export_voice(self, _tx_slot, _seq, _ambe):
        ...
        if _seq != ((_tx_slot.lastSeq + 1) & 0xFF):
            self._logger.warn(
                "(%s) Seq number not found. Got %d expected %d",
                self._system,
                _seq,
                _tx_slot.lastSeq + 1,
            )
    ...
    def send_fne_frame(self, _rx_slot, _flag, _dmr_frame):
        ...
        _rx_slot.frame_count += (
            1  # update count (used for stats and to make sure header was sent)
        )
        ...
        if hasattr(self._parent, "_peers"):
            ...
            for _peer in self._parent._peers:
                ...
                if _peerDict["TX_FREQ"] == _peerDict["RX_FREQ"]:
                    ...
                    if (
                        _rx_slot.stream_id != self._DMOStreamID
                    ):  # packet is from wrong stream?
                    ...
                    if (_frame[15] & 0x2F) == 0x22:  # call terminator flag?
                        ...
                        self._logger.info(
                            "(%s) DMO End of call, back to IDLE", self._system
                        )

                    _frame[15] = (
                        _frame[15] & 0x7F
                    ) | 0x80  # force to slot 2 if client in DMO mode
                else:
                    _frame[15] = _orig_flag  # Use the origional flag value if not DMO

                _repeaterID = int(_peerDict["PEER_ID"])
                for _index in range(
                    0, 4
    ...
    def make_dmrd(
        self, _seq, _src_id, _dst_id, _peer_id, _flag, _stream_id, _dmr_frame
    ):
    ...
    def encode_lc(self, _rx_slot, _dtype):
        ...
        # create lc
        lcHeader = (
            _flco + _fid + b"\x00" + short_to_bytes(_dst_id) + short_to_bytes(_src_id)
        )  # PF + Reserved + FLCO + FID + Service Options + Destination Address + Source Address

        _rx_slot.emblc = bptc.encode_emblc(
            lcHeader
        )  # save off the emb lc for voice frames B-E
    ...
    def encode_pi_header(self, _rx_slot):
        ...
        lcHeader = (
            _alg_id
            + _fid
            + short_to_bytes(_key_id)
            + _mi
            + short_to_bytes(_dst_id)
            + b"\x00\x00"
        )  # AlgID + FID + KeyID + MI + Destination Address + CRC-CCITT16
    ...
    def encode_voice(self, _ambe, _rx_slot):
        ...
        if _frame_type > 0:  # if not a SYNC frame cccxss
            index = (_rx_slot.cc << 3) | self.lcss[
                _frame_type
            ]  # index into the encode table makes this a simple lookup
            emb = bitarray(
                format(qr.ENCODE_1676[index], "016b")
            )  # create emb of 16 bits
            embedded = (
                emb[8:16] + _rx_slot.emblc[_frame_type] + emb[0:8]
            )  # Take emb and a chunk of the embedded LC and combine them into 48 bits
        else:
            embedded = MS_VOICE_SYNC  # Voice SYNC (48 bits)
        _new_frame = (
            _ambe[0:108] + embedded + _ambe[108:216]
        )  # Construct the dmr frame from AMBE(108 bits) + sync/emb (48 bits) + AMBE(108 bits)
        return _new_frame
...
class tlvIPSC(tlvBase):
    def __init__(self, _parent, _name, _config, _logger, _port):
        ...
        self.ipsc_seq = (
            0  # Same for all frames in a transmit session (sould use stream_id).  8 bit
        )
        pass
    ...
    def send_voice_header(self, _rx_slot):
        ...
        self.ipsc_seq = (
            self.ipsc_seq + 1
        ) & 0xFF  # this is an 8 bit value which wraps around.
        self.emb_lc = ""

        for i in range(0, 3):  # Output the 3 HEAD frames to our peers
            voiceHeader = self.generate_voice_header(
                _rx_slot, BURST_DATA_TYPE["VOICE_HEADER"]
            )
    ...
    def send_pi_header(self, _rx_slot):
        if _rx_slot.secure == True:
            voiceHeader = self.generate_voice_header(
                _rx_slot, BURST_DATA_TYPE["PI_HEADER"]
            )
    ...
    def send_voice72(self, _rx_slot, _ambe):
        ...
        burst = self.generate_ipsc_voice_burst(
            _rx_slot, BURST_DATA_TYPE["SLOT1_VOICE"], _ambe
        )
        ...
        _rx_slot.vf = (
            _rx_slot.vf + 1
        ) % 6  # the voice frame counter which is always mod 6
        pass
    ...
    def send_voice_term(self, _rx_slot):
        voiceHeader = self.generate_voice_header(
            _rx_slot, BURST_DATA_TYPE["VOICE_TERMINATOR"]
        )
    ...
    def send_ipsc(self, _slot, _frame):
        if (
            time() - self._parent._busy_slots[_slot]
        ) >= 0.10:  # slot is not busy so it is safe to transmit
            # Send the packet to all peers in the target IPSC
            self._parent.send_to_ipsc(_frame)
        else:
            self._logger.info(
                "Slot {} is busy, will not transmit packet from gateway".format(_slot)
            )
        self.rx[
            _slot
        ].frame_count += (
    ...
    def generate_ipsc_voice_header(self, _rx_slot):
        src_id = struct.pack(">I", _rx_slot.src_id)
        dst_id = struct.pack(">I", _rx_slot.dst_id)
        ...
        frame = (
            frameType
            + struct.pack(">I", _rx_slot.peer_id)
            + struct.pack("i", self.ipsc_seq)[0]
            + src_id[1]
            + src_id[2]
            + src_id[3]
            + dst_id[1]
            ...
            + dst_id[3]
            + CALL_PRIORITY_2
            + struct.pack(">I", _rx_slot.stream_id)
            + control
        )
    ...
    def generate_ipsc_burst(self, _rx_slot, _burst_type):
        ...
        bitLength = 0
        if (
            _burst_type == BURST_DATA_TYPE["VOICE_HEADER"]
            or _burst_type == BURST_DATA_TYPE["VOICE_TERMINATOR"]
        ):
            length = 10
        ...
        burst = (
            _burst_type
            + struct.pack("B", (_rx_slot.slot - 1) << 7)
            + struct.pack(">H", length)
            + _unk
            + struct.pack("B", syncType)
            + _unk
            + struct.pack("B", bitLength)
        )
        return burst
    ...
    def generate_ipsc_voice_burst(self, _rx_slot, _burst_type, _ambe):
        ...
        if _rx_slot.vf == 0:
            ...
            _rx_slot.vf == 1 or _rx_slot.vf == 2 or _rx_slot.vf == 3 or _rx_slot.vf == 5
        ):
    ...
    def generate_voice_header(self, _rx_slot, _burst_type):
        ...
        header = (
            struct.pack("B", headerType)
            + featureSet
            + struct.pack("B", svcOptions)
            + dst_id[1]
            + dst_id[2]
            + dst_id[3]
            + src_id[1]
    ...
    def generate_pi_header(self, _rx_slot, _burst_type):
        ...
        header = (
            struct.pack("B", _rx_slot.alg_id)
            + featureSet
            + struct.pack("B", _rx_slot.key_id)
            + mi[0]
            + mi[1]
            + mi[2]
            + mi[3]
