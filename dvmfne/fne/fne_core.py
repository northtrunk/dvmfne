...
def setup_fne():
    ...
    def sig_handler(_signal, _frame):
        logger.info(
            "Digital Voice Modem FNE is terminating with signal %s", str(_signal)
        )
...
def setup_activity_log(_config, _logger):
    ...
    logsplitter = task.LoopingCall(
        act_log_split_loop, act_log_file, _config["Log"]["ActivityLogFile"]
    )
...
class AMBE:
    ...
    def parse_ambe(self, _client, _data):
        ...
        self._logger.debug(
            "(%s) SEQ %d SRC_ID %d DST_ID %d PEER %d BITS %0X TS %d CALLTYPE %d FRAMETYPE %d VOICESEQ %d [STREAM ID %0X]",
            _client,
            _seq,
            _srcID,
            _dstID,
    ...
    def __init__(self, _fne, _config, _logger):
        ...
        class UDP_IMPORT(DatagramProtocol):
            ...
            def datagramReceived(
                self, _data, hostInfo
            ):  # hostInfo is tuple; converted from 2.x to 3.x syntax
                self.func(_data, hostInfo)

        self.udp_port = reactor.listenUDP(
            self._port, UDP_IMPORT(self.packet_datagramReceived)
        )
...
class coreFNE(DatagramProtocol):
    ...
    def startProtocol(self):
        ...
        self._system_maintenance_loop = self._system_maintenance.start(
            self._CONFIG["Global"]["PingTime"]
        )
    ...
    def send_peer(self, _peer, _packet):
        ...
        if self._CONFIG["Log"]["RawPacketTrace"]:
            self._logger.debug(
                "(%s) PEER %s Network Transmitted (to %s:%s) -- %s",
                self._system,
                self._peers[_peer]["PEER_ID"],
                self._peers[_peer]["IP"],
                self._peers[_peer]["PORT"],
                ahex(_packet),
            )

    def send_master(self, _packet):
        self.transport.write(
            _packet, (self._config["MasterAddress"], self._config["MasterPort"])
        )
        if self._CONFIG["Log"]["RawPacketTrace"]:
            self._logger.debug(
                "(%s) Network Transmitted (to %s:%s) -- %s",
                self._system,
                self._config["MasterAddress"],
                self._config["MasterPort"],
                ahex(_packet),
            )
    ...
    def master_dereg(self):
        for _peer in self._peers:
            ...
            self._logger.info(
                "(%s) De-Registration sent to PEER %s",
                self._system,
                self._peers[_peer]["PEER_ID"],
            )
    ...
    def peer_dereg(self):
        self.send_master(
            fne_const.TAG_REPEATER_CLOSING + int_to_bytes(self._config["PeerId"])
        )
        self._logger.info(
            "(%s) De-Registration sent to MASTER (%s:%s)",
            self._system,
            self._config["MasterAddress"],
            self._config["MasterPort"],
        )
    ...
    def peer_trnslog(self, _message):
        self.send_master(
            fne_const.TAG_TRANSFER_ACT_LOG
            + int_to_bytes(self._config["PeerId"])
            + _message
        )
    ...
    def send_peer_wrids(self, _peer, _rids):
        ...
        if self._config["Mode"] == "master":
            ...
            self._logger.debug(
                "(%s) Whitelist RIDs sent to PEER %s",
                self._system,
                self._peers[_peer]["PEER_ID"],
            )
    ...
    def send_peer_brids(self, _peer, _rids):
        ...
        if self._config["Mode"] == "master":
            ...
            self._logger.debug(
                "(%s) Blacklist RIDs sent to PEER %s",
                self._system,
                self._peers[_peer]["PEER_ID"],
            )
    ...
    def send_peer_disabled_tgids(self, _peer, _tgids):
        ...
        if self._config["Mode"] == "master":
            ...
            self._logger.debug(
                "(%s) Deactivated TGIDs sent to PEER %s",
                self._system,
                self._peers[_peer]["PEER_ID"],
            )
    ...
    def master_maintenance_loop(self):
        for _peer in self._peers:
            ...
            # longer than allowed
            if (
                _this_peer["LAST_PING"]
                + self._CONFIG["Global"]["PingTime"]
                * self._CONFIG["Global"]["MaxMissed"]
                < time()
            ):
                self._logger.info(
                    "(%s) PEER %s has timed out", self._system, _this_peer["PEER_ID"]
                )
    ...
    def peer_maintenance_loop(self):
        ...
        if (
            ...
            or self._stats["CONNECTION"] == "RPTL_SENT"
        ):
            self._stats["PINGS_SENT"] = 0
            self._stats["PINGS_ACKD"] = 0
            self._stats["CONNECTION"] = "RPTL_SENT"
            self.send_master(
                fne_const.TAG_REPEATER_LOGIN + int_to_bytes(self._config["PeerId"])
            )
            self._logger.info(
                "(%s) Sending login request to MASTER (%s:%s)",
                self._system,
                self._config["MasterAddress"],
                self._config["MasterPort"],
            )
        ...
        if self._stats["CONNECTION"] == "YES":
            self.send_master(
                fne_const.TAG_REPEATER_PING + int_to_bytes(self._config["PeerId"])
            )
            self._stats["PINGS_SENT"] += 1
            self._logger.debug(
                "(%s) RPTPING Sent to MASTER. Pings since connected: %s",
                self._system,
                self._stats["PINGS_SENT"],
            )

    # Aliased in __init__ to datagramReceived if system is a master
    def master_datagramReceived(
        self, _data, hostInfo
    ):  # hostInfo is a tuple; converted from 2.x to 3.x syntax
        ...
        # print(_data)
        if (
            _data[:4] == fne_const.TAG_DMR_DATA
        ):  # fne_const.TAG_DMR_DATA -- encapsulated DMR data frame
            _peer_id = bytes_to_int(_data[11:15])
            if (
                _peer_id in self._peers
                and self._peers[_peer_id]["CONNECTION"] == "YES"
                and self._peers[_peer_id]["IP"] == _host
                and self._peers[_peer_id]["PORT"] == _port
            ):
                _seq = _data[4]
                ...
                if (
                    ...
                    if self._config["PacketData"]:
                        if (_frame_type == fne_const.FT_DATA_SYNC) and (
                            (_dtype_vseq == fne_const.DT_CSBK)
                            or (_dtype_vseq == fne_const.DT_DATA_HEADER)
                        ...
                        for _peer in self._peers:
                            if _peer != _peer_id:
                                if (
                                    ...
                                    self._logger.debug(
                                        "(%s) DMRD: Packet TS %s SRC_PEER %s DST_ID %s DST_PEER %s [STREAM ID %s]",
                                        self._system,
                                        _slot,
                                        _peer_id,
                                        _dst_id,
            ...
            _data[:4] == fne_const.TAG_P25_DATA
        ):  # fne_const.TAG_P25_DATA -- encapsulated P25 data frame
            _peer_id = bytes_to_int(_data[11:15])
            if (
                _peer_id in self._peers
                and self._peers[_peer_id]["CONNECTION"] == "YES"
                ...
                _dst_id = bytes_to_int(_data[8:11])
                _call_type = (
                    "unit" if (_data[4] == fne_const.P25_LC_PRIVATE) else "group"
                )
                _duid = _data[22]
                _dtype_vseq = (
                    fne_const.FT_VOICE
                    ...
                    else fne_const.DT_TERMINATOR_WITH_LC
                )
                _stream_id = bytes_to_int(_data[16:20])
                ...
                if (
                    ...
                    # If packet data exporting is configured...
                    if self._config["PacketData"]:
                        if (_duid == fne_const.P25_DUID_TSDU) or (
                            _duid == fne_const.P25_DUID_PDU
                        ):
                            self._packet_data.send_data(_data)
                        ...
                        for _peer in self._peers:
                            if _peer != _peer_id:
                                if (
                                    ...
                                    self.send_peer(_peer, _data)
                                    self._logger.debug(
                                        "(%s) P25D: Packet SRC_PEER %s DST_ID %s DST_PEER %s [STREAM ID %s]",
                                        self._system,
                                        _peer_id,
                                        _dst_id,
                                        _peer,
            ...
            _data[:4] == fne_const.TAG_REPEATER_LOGIN
        ):  # fne_const.TAG_REPEATER_LOGIN -- a repeater wants to login
            _peer_id = bytes_to_int(_data[4:8])
            if _peer_id:
                ...
                self._logger.info(
                    "(%s) Repeater logging in with PEER %s, %s:%s",
                    self._system,
                    _peer_id,
                    _host,
                    _port,
                ...
                self._logger.info(
                    "(%s) Sent Challenge Response to PEER %s for login %s",
                    self._system,
                    _peer_id,
                ...
                self.transport.write(
                    fne_const.TAG_MASTER_NAK + _peer_id, (_host, _port)
                )
                self._logger.warning(
                    "(%s) Invalid login from PEER %s", self._system, _peer_id
                )

        elif (
            _data[:4] == fne_const.TAG_REPEATER_AUTH
        ):  # fne_const.TAG_REPEATER_AUTH -- Repeater has answered our login challenge
            _peer_id = bytes_to_int(_data[4:8])
            ...
            if (
                ...
                _calc_hash = sha256(
                    _salt_str + self._config["Passphrase"].encode()
                ).digest()
                if _sent_hash == _calc_hash:
                    ...
                    self._logger.warning(
                        "(%s) PEER %s has FAILED the login exchange",
                        self._system,
                        _this_peer["PEER_ID"],
                    )
                    self.transport.write(
                        fne_const.TAG_MASTER_NAK + _peer_bytes, (_host, _port)
                    )
                ...
                self._logger.warning(
                    "(%s) RPTK from unauth PEER %s", self._system, _peer_id
                )

        elif (
            _data[:4] == fne_const.TAG_REPEATER_CONFIG
        ):  # fne_const.TAG_REPEATER_CONFIG -- Repeater is sending it's configuration
            _peer_id = bytes_to_int(_data[4:8])
            if (
                _peer_id in self._peers
                ...
                self.send_peer(
                    _peer_id, fne_const.TAG_REPEATER_ACK + _peer_id.to_bytes(4, "big")
                )
                self._logger.info(
                    "(%s) PEER %s has sent configuration",
                    self._system,
                    _this_peer["PEER_ID"],
                )
                self._logger.info(
                    "(%s) PEER %s Connection from PEER Completed",
                    self._system,
                    _this_peer["PEER_ID"],
                )
                ...
                self.transport.write(
                    fne_const.TAG_MASTER_NAK + _peer_id.to_bytes(4, "big"),
                    (_host, _port),
                )
                self._logger.warning(
                    "(%s) Configuration from unauth PEER %s", self._system, _peer_id
                )

        elif (
            _data[:5] == fne_const.TAG_REPEATER_CLOSING
        ):  # fne_const.TAG_REPEATER_CLOSING -- Disconnect command
            _peer_id = bytes_to_int(_data[5:9])
            if (
                _peer_id in self._peers
                and self._peers[_peer_id]["CONNECTION"] == "YES"
                and self._peers[_peer_id]["IP"] == _host
                ...
                self._logger.info(
                    "(%s) PEER %s is closing down", self._system, _peer_id
                )
                self.transport.write(
                    fne_const.TAG_MASTER_NAK + _peer_id.to_bytes(4, "big"),
                    (_host, _port),
            ...
            _data[:7] == fne_const.TAG_REPEATER_PING
        ):  # fne_const.TAG_REPEATER_PING -- peer is pinging us
            _peer_id = bytes_to_int(_data[7:11])
            if (
                _peer_id in self._peers
                and self._peers[_peer_id]["CONNECTION"] == "YES"
                and self._peers[_peer_id]["IP"] == _host
                ...
                self.send_peer(
                    _peer_id, fne_const.TAG_MASTER_PONG + _peer_id.to_bytes(4, "big")
                )
                self._logger.debug(
                    "(%s) Received and answered RPTPING from PEER %s",
                    self._system,
                    _peer_id,
                )
            else:
                ...
                self._logger.warning(
                    "(%s) RPTPING from unauth PEER %s", self._system, _peer_id
                )

        elif (
            _data[:7] == fne_const.TAG_TRANSFER_ACT_LOG
        ):  # fne_const.TAG_TRANSFER_ACT_LOG -- peer is transferring activity log data to us
            ...
            _data[:8] == fne_const.TAG_TRANSFER_DIAG_LOG
        ):  # fne_const.TAG_TRANSFER_DIAG_LOG -- peer is transferring diagnostics log data to us
            if self._CONFIG["Log"]["AllowDiagTrans"] == True:
                _peer_id = bytes_to_int(_data[8:12])
                if (
                    _peer_id in self._peers
                    and self._peers[_peer_id]["CONNECTION"] == "YES"
                    and self._peers[_peer_id]["IP"] == _host
                    ...
                    diag_log_file = get_peer_diag_log_handler(
                        self._CONFIG, self._logger, _peer_id
                    )
            ...
            try:
                self._logger.error(
                    "(%s) Unrecognized command PEER %s PACKET %s",
                    self._system,
                    _peer_id,
                    ahex(_data),
                )
                ...
                self._logger.error(
                    "(%s) Unrecognized command %s PACKET %s",
                    self._system,
                    _data[:9],
                    ahex(_data),
                )
    ...
    def peer_datagramReceived(
        ...
        if (
            self._config["MasterAddress"] == _host
            and self._config["MasterPort"] == _port
        ):
            # process opcode from data, usually first 4 bytes but can be a varied length
            # depending on the opcode
            if (
                ...
                if (
                    _peer_id == self._config["PeerId"]
                ):  # Validate the source and intended target
                    _seq = _data[4:5]
                    _rf_src = bytes_to_int(_data[5:8])
                    _dst_id = bytes_to_int(_data[8:11])
                    _bits = _data[15]
                ...
                _data[:4] == fne_const.TAG_P25_DATA
            ):  # fne_const.TAG_P25_DATA -- encapsulated P25 data
                _peer_id = bytes_to_int(_data[11:15])
                if _peer_id != self._config["PeerId"]:
                    # self._logger.warning('(%s) PEER %s; routed traffic, rewriting PEER %s', self._system, _peer_id, self._config['PeerId'])
                    _peer_id = self._config["PeerId"]
                ...
                _data[:5] == fne_const.TAG_MASTER_CLOSING
            ):  # MSTCL -- notify us the master is closing down
                if bytes_to_int(_data[5:9]) == self._config["PeerId"]:
                    self._stats["CONNECTION"] = "NO"
                    self._logger.info(
                        "(%s) PEER %s MSTCL recieved",
                        self._system,
                        self._config["PeerId"],
                    )

            elif (
                _data[:6] == fne_const.TAG_MASTER_NAK
            ):  # fne_const.TAG_MASTER_NAK -- a NACK from the master
                _peer_id = bytes_to_int(_data[6:10])
                if (
                    _peer_id == self._config["PeerId"]
                ):  # Validate the source and intended target
                    self._logger.warning(
                        "(%s) PEER %s MSTNAK received",
                        self._system,
                        self._config["PeerId"],
                    )
                    self._stats[
                        "CONNECTION"
                    ] = "NO"  # Disconnect ourselves and re-register

            elif (
                _data[:6] == fne_const.TAG_REPEATER_ACK
            ):  # fne_const.TAG_REPEATER_ACK -- an ACK from the master
                # Depending on the state, an RPTACK means different things, in
                # each clause, we check
                # and/or set the state
                if (
                    self._stats["CONNECTION"] == "RPTL_SENT"
                ):  # If we've sent a login request...
                    _login_int32 = bytes_to_int(_data[6:10])
                    self._logger.info(
                        "(%s) PEER %s login ACK received with ID %s",
                        self._system,
                        self._config["PeerId"],
                        _login_int32,
                    )
                    _pass_hash = sha256(
                        int_to_bytes(_login_int32) + self._config["Passphrase"].encode()
                    ).hexdigest()
                    _pass_hash = bhex(_pass_hash)
                    self.send_master(
                        fne_const.TAG_REPEATER_AUTH
                        + int_to_bytes(self._config["PeerId"])
                        + _pass_hash
                    )
                    ...
                    self._stats["CONNECTION"] == "AUTHENTICATED"
                ):  # If we've sent the login challenge...
                    if bytes_to_int(_data[6:10]) == self._config["PeerId"]:
                        self._logger.info(
                            "(%s) PEER %s authentication accepted",
                            self._system,
                            self._config["PeerId"],
                        )
                        _peer_config = {
                            ...
                            "rcon": {"password": "ABCD123", "port": 0},
                        }

                        _config_packet = json.dumps(
                            _peer_config, separators=(",", ":")
                        ).encode()

                        self.send_master(
                            fne_const.TAG_REPEATER_CONFIG
                            + int_to_bytes(self._config["PeerId"])
                            + _config_packet
                        )
                        self._stats["CONNECTION"] = "CONFIG-SENT"
                        self._logger.info(
                            "(%s) PEER %s Configuration sent to master",
                            self._system,
                            self._config["PeerId"],
                        )
                    else:
                        self._stats["CONNECTION"] = "NO"
                        self._logger.error(
                            "(%s) PEER %s Configuration master ACK Contained peer wrong ID - Connection Reset",
                            self._system,
                            self._config["PeerId"],
                        )
                    ...
                    self._stats["CONNECTION"] == "CONFIG-SENT"
                ):  # If we've sent out configuration to the master
                    if bytes_to_int(_data[6:10]) == self._config["PeerId"]:
                        self._logger.info(
                            "(%s) PEER %s Master accepted configuration",
                            self._system,
                            self._config["PeerId"],
                        )
                        self._stats["CONNECTION"] = "YES"
                        self._logger.info(
                            "(%s) PEER %s Connection to MASTER Completed",
                            self._system,
                            self._config["PeerId"],
                        )
                    else:
                        self._stats["CONNECTION"] = "NO"
                        self._logger.error(
                            "(%s) PEER %s Master ACK Contained wrong peer ID - Connection Reset",
                            self._system,
                            self._config["PeerId"],
                        )

            elif (
                _data[:7] == fne_const.TAG_MASTER_PONG
            ):  # fne_const.TAG_MASTER_PONG -- a reply to RPTPING (send by peer)
                if bytes_to_int(_data[7:11]) == self._config["PeerId"]:
                    self._stats["PINGS_ACKD"] += 1
                    self._logger.debug(
                        "(%s) PEER %s MSTPONG received, pongs since connected %s",
                        self._system,
                        self._config["PeerId"],
                        self._stats["PINGS_ACKD"],
                    )
...
class report(NetstringReceiver):
    ...
    def connectionMade(self):
        ...
        self._factory._logger.info(
            "Reporting client connected: %s", self.transport.getPeer()
        )

    def connectionLost(self, reason):
        self._factory._logger.info(
            "Reporting client disconnected: %s", self.transport.getPeer()
        )
    ...
    def process_message(self, _message):
        ...
        if opcode == REPORT_OPCODES["CONFIG_REQ"]:
            self._factory._logger.info(
                "Reporting client sent 'CONFIG_REQ': %s", self.transport.getPeer()
            )
            self.send_config()
        elif opcode == REPORT_OPCODES["RCON_REQ"]:
            _arguments = _message.split(",")
            if len(_arguments) < 6:
                self._factory._logger.error(
                    "RCON request contained an invalid number of arguments; RCON_REQ from %s",
            ...
            try:
                ...
                self._factory._logger.error(
                    "RCON request contained invalid arguments; RCON_REQ from %s",
                    self.transport.getPeer(),
                )
                return
            ...
            if not _peer:
                self._factory._logger.error(
                    "RCON request contained invalid PEER ID; RCON_REQ from %s, PEER ID %s",
                    self.transport.getPeer(),
                    _peer_id,
                )
                return
            ...
            if self._factory._config["Global"]["RconTool"] != "":
                self._factory._logger.info(
                    "RCON_REQ from %s: PEER ID %s COMMAND %s DMR SLOT %s ARGUMENT %s MOT MFID %s",
                    self.transport.getPeer(),
                    _peer_id,
                    _command,
                    _dmr_slot,
                ...
                _root_cmd = [
                    self._factory._config["Global"]["RconTool"],
                    "-a",
                    str(_peer_ip),
                    "-p",
                    str(_rcon_port),
            ...
            self._factory._logger.error(
                "Report unrecognized opcode %s PACKET %s", opcode, ahex(_message)
            )
...
class reportFactory(Factory):
    ...
    def buildProtocol(self, addr):
        if (addr.host) in self._config["Reports"][
            "ReportClients"
        ] or "*" in self._config["Reports"]["ReportClients"]:
            self._logger.debug(
