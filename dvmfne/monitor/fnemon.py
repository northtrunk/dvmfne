    ...
    try:
        with open(_file, "r") as log:
            ...
            for line in rev_log:
                if (
                    re.search(
                        "(RF voice|RF encrypted voice|RF late entry|RF data|RF voice rejection)",
                        line,
                    )
                    != None
                    and re.search("(group grant|unit-to-unit grant)", line) != None
                    and re.search(
                        "(unit registration|group affiliation|unit deregistration|location registration request)",
                        line,
                    )
                    != None
                    and re.search(
                        "(status update|message update|call alert|ack response)", line
                    )
                    != None
                    and re.search(
                        "(cancel service|radio check|radio inhibit|radio uninhibit)",
                        line,
                    )
                    == None
                ):
                ...
                if (
                    re.search("(voice transmission|voice header|late entry)", line)
                    != None
                ):
                ...
                if (
                    re.search(
                        "(unit registration|group affiliation|unit deregistration|location registration)",
                        line,
                    )
                    != None
                    or re.search("(status update|message update|cancel service)", line)
                    != None
                ):
                    if _to == "":
                ...
                if (
                    ...
                    for etLine in fwd_log[lineIdx:]:
                        ...
                        if (
                            re.search(
                                "(RF end of|ended RF data transmission|transmission lost)",
                                etLine,
                            )
                            == None
                        ):
...
def gen_activity():
    ...
    dashboard_server.broadcast(
        WEBSOCK_OPCODES["ACTIVITY"] + json.dumps(_entries).encode()
    )
        ...
        if _hbp_data["Enabled"] == True:
            if _hbp_data["Mode"] == "master":
                ...
                for _peer in _hbp_data["PEERS"]:
                    ...
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "CONNECTION"
                    ] = _hbp_data["PEERS"][_peer]["CONNECTION"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "PINGS_RECEIVED"
                    ] = _hbp_data["PEERS"][_peer]["PINGS_RECEIVED"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "LAST_PING"
                    ] = _hbp_data["PEERS"][_peer]["LAST_PING"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "IP"
                    ] = _hbp_data["PEERS"][_peer]["IP"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "PORT"
                    ] = _hbp_data["PEERS"][_peer]["PORT"]

                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "IDENTITY"
                    ] = _hbp_data["PEERS"][_peer]["IDENTITY"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "RX_FREQ"
                    ] = _hbp_data["PEERS"][_peer]["RX_FREQ"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "TX_FREQ"
                    ] = _hbp_data["PEERS"][_peer]["TX_FREQ"]

                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "LATITUDE"
                    ] = _hbp_data["PEERS"][_peer]["LATITUDE"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "LONGITUDE"
                    ] = _hbp_data["PEERS"][_peer]["LONGITUDE"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "HEIGHT"
                    ] = _hbp_data["PEERS"][_peer]["HEIGHT"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "LOCATION"
                    ] = _hbp_data["PEERS"][_peer]["LOCATION"]

                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "TX_OFFSET"
                    ] = _hbp_data["PEERS"][_peer]["TX_OFFSET"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "CH_BW"
                    ] = _hbp_data["PEERS"][_peer]["CH_BW"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "CHANNEL_ID"
                    ] = _hbp_data["PEERS"][_peer]["CHANNEL_ID"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "CHANNEL_NO"
                    ] = _hbp_data["PEERS"][_peer]["CHANNEL_NO"]
                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "TX_POWER"
                    ] = _hbp_data["PEERS"][_peer]["TX_POWER"]

                    _stats_table["MASTERS"][_hbp]["PEERS"][int_id(_peer)][
                        "SOFTWARE_ID"
                    ] = _hbp_data["PEERS"][_peer]["SOFTWARE_ID"]
...
def process_message(_message):
    ...
    if opcode == REPORT_OPCODES["CONFIG_RSP"]:
        ...
        if p[0] == "GROUP VOICE":
            if p[1] == "END":
                log_message = "[{}] ({}) {} {}: System: {}; Peer: {}; Subscriber: {}; TS: {}; TGID: {}; Duration: {}s".format(
                    _now, p[2], p[0], p[1], p[3], p[5], p[6], p[7], p[8], p[9]
                )
            elif p[1] == "START":
                log_message = "[{}] ({}) {} {}: System: {}; Peer: {}; Subscriber: {}; TS: {}; TGID: {}".format(
                    _now, p[2], p[0], p[1], p[3], p[5], p[6], p[7], p[8]
                )
            elif p[1] == "END WITHOUT MATCHING START":
                log_message = "[{}] ({}) {} {} on System {}: Peer: {}; Subscriber: {}; TS: {}; TGID: {}".format(
                    _now, p[2], p[0], p[1], p[3], p[5], p[6], p[7], p[8]
                )
            else:
            ...
            log_message = (
                "[{}] ({}) {} {}: System: {}; Target: {}; TS: {}; TGID: {}".format(
                    _now, p[2], p[0], p[1], p[3], p[4], p[5], p[6]
                )
            )
        elif p[0] == "REJECT ACL":
            log_message = "[{}] ({}) {} {}: System: {}; Peer: {}; Subscriber: {}; TS: {}; TGID: {}".format(
            ...
            if p[1] == "ADJ STS BCS":
                log_message = "[{}] ({}) {} {}: System: {}; Peer: {}".format(
                    _now, p[2], p[0], p[1], p[3], p[5]
                )
            else:
                log_message = "[{}] ({}) {} {}: System: {}; Peer: {}; Subscriber: {}; TS: {}; TGID: {}".format(
                    _now, p[2], p[0], p[1], p[3], p[5], p[6], p[7], p[8]
            ...
            log_message = (
                "[{}] ({}) {} {}: System: {}; Peer: {}; Subscriber: {}; TS: {}".format(
                    _now, p[2], p[0], p[1], p[3], p[5], p[6], p[7]
                )
            )
        else:
...
class reportClientFactory(ReconnectingClientFactory):
    ...
    def startedConnecting(self, connector):
        ...
        if "dashboard_server" in locals() or "dashboard_server" in globals():
            dashboard_server.broadcast(
                WEBSOCK_OPCODES["QUIT"] + b"Connection to FNE Established"
            )
    ...
    def onMessage(self, payload, isBinary):
        ...
        if isBinary:
            ...
            if _opcode == WEBSOCK_OPCODES["MESSAGE"]:
                ...
                logging.info(
                    "Received system command: PEER ID %s COMMAND %s DMR SLOT %s ARGUMENT %s MOT MFID %s",
                    _peer_id,
                    _command,
                    _dmr_slot,
                    _command_arg,
                ...
                self.sendMessage(
                    WEBSOCK_OPCODES["DIAG_LOG"] + json.dumps(diag_log).encode()
                )
            else:
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=logging.INFO,
        handlers=[
            logging.FileHandler(config.PATH + "logfile.log"),
